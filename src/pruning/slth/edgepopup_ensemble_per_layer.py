...
class SubnetLinear(nn.Linear):
    ...
    def init_sub_scores(self, mode="kaiming_normal"):
        if mode == "kaiming_uniform":
            nn.init.kaiming_uniform_(self.sub_scores, a=math.sqrt(5))
        elif mode == "kaiming_normal":
            nn.init.kaiming_normal_(self.sub_scores)
        elif mode == "xavier_uniform":
            nn.init.xavier_uniform_(self.sub_scores)
        elif mode == "xavier_normal":
            nn.init.xavier_normal_(self.sub_scores)
        elif mode == "uniform":
            nn.init.uniform_(self.sub_scores, a=-1, b=1)
        elif mode == "normal":
            nn.init.normal_(self.sub_scores)
        else:
            raise ValueError(f"Unknown initialization mode: {mode}")
    ...
    def forward(self, x):
        scores = self.alpha * self.scores + (1 - self.alpha) * self.sub_scores
        subnet = self.subnet_func.apply(scores.abs(), self.remain_rate)
