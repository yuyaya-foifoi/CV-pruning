...
class SubnetLinear(nn.Linear):
    ...
    def set_remain_rate(self):
        self.k = nn.Parameter(torch.tensor(1.0))

    def forward(self, x):
        k = 0.999 * torch.sigmoid(self.k) + 0.001
        subnet = self.subnet_func.apply(self.clamped_scores, k)
...
class SubnetConv(nn.Conv2d):
    ...
    def forward(self, x):
        k = 0.99 * torch.sigmoid(self.k) + 0.01
        subnet = self.subnet_func.apply(self.clamped_scores, k)
        w = self.weight * subnet
        x = F.conv2d(
            x,
...
def modify_module_for_slth(
    net, init_mode=None, init_scores_mode="kaiming_uniform"
):
